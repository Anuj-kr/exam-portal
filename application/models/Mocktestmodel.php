...
class Mocktestmodel extends CI_Model
{
	...
	public function add_user($d,$user_id, $test_types, $test_id)  //add new user in mocktest table
	{
		$q = $this->db->where(['user_id'=>$user_id,'test_type'=>$test_types,'test_id'=>$test_id])
						->get('user_session');
	...
	public function save_ans($ans,$course,$c_testid,$c_q_no,$user)
	{
		...
		else 
		{
			...
			return $this->db->insert('mocktest_answers',$data); // //if ans is not saved inserting new ans
		}	
	...
	public function save_time($user_id,$test_id,$time)
	{
